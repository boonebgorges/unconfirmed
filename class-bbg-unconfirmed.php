...
class BBG_Unconfirmed {
	...
	function __construct() {
		add_filter( 'bbg_cpt_pag_add_args', [ $this, 'add_args' ] );

		add_filter( 'boones_sortable_columns_keys_to_remove', [ $this, 'sortable_keys_to_remove' ] );

		add_filter( 'map_meta_cap', [ $this, 'map_moderate_signups_cap' ], 10, 4 );
		...
		add_action( $admin_hook, [ $this, 'add_admin_panel' ] );
	}
	...
	function add_admin_panel() {
		$page = add_submenu_page( 'users.php', esc_html__( 'Unconfirmed', 'unconfirmed' ), esc_html__( 'Unconfirmed', 'unconfirmed' ), 'moderate_signups', 'unconfirmed', [ $this, 'admin_panel_main' ] );
		...
		if ( isset( $_REQUEST['unconfirmed_action'] ) ) {
			$action = sanitize_text_field( wp_unslash( $_REQUEST['unconfirmed_action'] ) );
	 ...
	 *
	 * @return array Possibly modified user capabilities.
	 */
	...
	function setup_users( $args ) {
		...
		if ( $this->is_multisite ) {
			...
			if ( 'user_registered' === $parsed_args['orderby'] ) {
				$parsed_args['orderby'] = 'registered';
			} elseif ( 'user_activation_key' === $parsed_args['orderby'] ) {
				$parsed_args['orderby'] = 'activation_key';
			}

			$sql['orderby'] = 'ORDER BY ' . sanitize_sql_orderby( $parsed_args['orderby'] );
			$sql['order']   = strtoupper( $parsed_args['order'] );
			$sql['limit']   = $wpdb->prepare( 'LIMIT %d, %d', $parsed_args['offset'], $parsed_args['number'] );
		} else {
			...
			if ( 'registered' === $parsed_args['orderby'] ) {
				$parsed_args['orderby'] = 'user_registered';
			} elseif ( 'activation_key' === $parsed_args['orderby'] ) {
				$parsed_args['orderby'] = 'um.activation_key';
			}
		...
		$prepared_paged_query = apply_filters( 'unconfirmed_paged_query', join( ' ', $sql ), $sql, $args, $parsed_args );

		// phpcs:ignore
		$users = $wpdb->get_results( $prepared_paged_query );
		...
		foreach ( (array) $users as $key => $user ) {
			...
			if ( ! ( $this->is_multisite ) && $user->user_registered ) {
		...
		$total_query   = apply_filters( 'unconfirmed_total_query', join( ' ', $sql ), $sql, $parsed_args, $args );
	...
	/**
	 * Get all keys to remove from the base URL.
	 *
	 * @param array $keys Initial set of keys.
	 *
	 * @return array Keys with added plugin keys.
	 */
	 ...
	 *
	 * @return stdClass Database query result.
	 *
	 * @package Unconfirmed
	 * @since 1.2
	 */
	function get_userdata_from_key( $key ) {
		...
		// phpcs:ignore
		return $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->users WHERE ID = %d", (int) $user_id ) );
	}
	...
	function activate_user() {
		...
		$keys = array_map( 'sanitize_text_field', wp_unslash( (array) $_REQUEST['unconfirmed_key'] ) );
		...
		foreach ( (array) $keys as $key ) {
			if ( $this->is_multisite ) {
				...
				if ( empty( $user_id ) ) {
					return new WP_Error( 'invalid_key', esc_html__( 'Invalid activation key', 'unconfirmed' ) );
				}
	...
	function delete_user() {
		...
		foreach ( (array) $keys as $key ) {
			if ( $this->is_multisite ) {
				// Ensure the user exists before deleting, and pass the data along to a hook.
				// phpcs:ignore
	...
	function resend_email() {
		...
		foreach ( $keys as $key ) {
			...
			if ( $this->is_multisite ) {
				...
				// If you're running BP on a non-multisite instance of WP, use the BP function to send the email.
	...
	/**
	 * Redirects the user after the requesting actions have been performed,
	 *
	...
	function do_redirect() {
		$query_vars = [ 'unconfirmed_complete' => '1' ];
	 ...
	 *
	 * @param array $add_args Existing arguments.
	 *
	 * @return array Arguments with search added.
	 */
	function add_args( $add_args ) {
		// phpcs:disable WordPress.CSRF.NonceVerification
		if ( ! empty( $_REQUEST['s'] ) ) {
			$search_text   = rawurlencode( trim( sanitize_text_field( wp_unslash( $_REQUEST['s'] ) ) ) );
	...
	function setup_get_users() {
		...
		// phpcs:disable WordPress.CSRF.NonceVerification
		foreach ( $_REQUEST as $unsafe_get_key => $unsafe_activation_keys ) {
			// phpcs:enable

			$safe_get_key = explode( '_', sanitize_text_field( wp_unslash( $unsafe_get_key ) ) );

			if ( 'updated' === $safe_get_key[0] || 'error' === $safe_get_key[0] ) {
				$safe_activation_keys = explode( ',', sanitize_text_field( wp_unslash( $unsafe_activation_keys ) ) );

				if ( $this->is_multisite ) {
					foreach ( (array) $safe_activation_keys as $ak_index => $activation_key ) {
						$safe_activation_keys[ $ak_index ] = '"' . sanitize_text_field( $activation_key ) . '"';
					}
					$safe_activation_keys = implode( ',', $safe_activation_keys );

					// phpcs:ignore
					$registrations = $wpdb->get_results( $wpdb->prepare( "SELECT user_email, activation_key FROM $wpdb->signups WHERE activation_key IN (%s)", $safe_activation_keys ) );
				} else {
					$registrations = [];
					foreach ( (array) $safe_activation_keys as $akey ) {
				...
				$updated_or_error = $safe_get_key[0];
				$message_type     = $safe_get_key[1];
		...
		if ( ! empty( $this->results_emails ) ) {
			...
			if ( ! empty( $this->results_emails['updated'] ) ) {
				foreach ( $this->results_emails['updated'] as $message_type => $registrations ) {
					...
					switch ( $message_type ) {
						case 'activated':
							// translators: %s list of emails.
							$message = sprintf( esc_html__( 'You successfully activated the following users: %s', 'unconfirmed' ), $emails );
							break;

						case 'resent':
							// translators: %s list of emails.
							$message = sprintf( esc_html__( 'You successfully resent activation emails to the following users: %s', 'unconfirmed' ), $emails );
							break;

						case 'deleted':
							if ( count( $registrations ) > 1 ) {
								$message = esc_html__( 'Registrations successfully deleted.', 'unconfirmed' );
							} else {
								$message = esc_html__( 'Registration successfully deleted.', 'unconfirmed' );
							}
							break;
			...
			if ( ! empty( $this->results_emails['error'] ) ) {
				foreach ( $this->results_emails['error'] as $message_type => $registrations ) {
					...
					switch ( $message_type ) {
						case 'nokey':
							$message = esc_html__( 'You didn\'t provide an activation key.', 'unconfirmed' );
							break;

						case 'couldntactivate':
							// translators: %s list of emails.
							$message = sprintf( esc_html__( 'The following users could not be activated: %s', 'unconfirmed' ), $emails );
							break;

						case 'nouser':
							$message = esc_html__( 'You provided invalid activation keys.', 'unconfirmed' );
							break;

						case 'unsent':
							// translators: %s list of emails.
							$message = sprintf( esc_html__( 'Activations emails could not be resent to the following email addresses: %s', 'unconfirmed' ), $emails );
							break;
	...
	function admin_panel_main() {
		...
		if ( ! class_exists( 'BBG_CPT_Pag' ) ) {
			require_once( __DIR__ . '/lib/class-bbg-cpt-pag.php' );
		}
		$pagination = new BBG_CPT_Pag;

		// Load the sortable helper.
		if ( ! class_exists( 'BBG_CPT_Sort' ) ) {
			require_once( dirname( __FILE__ ) . '/lib/class-bbg-cpt-sort.php' );
		}

		$cols = [
			[
				'name'      => 'user_login',
				'title'     => esc_html__( 'User Login', 'unconfirmed' ),
				'css_class' => 'login',
			],
			[
				'name'      => 'user_email',
				'title'     => esc_html__( 'Email Address', 'unconfirmed' ),
				'css_class' => 'email',
			],
			[
				'name'      => 'registered',
			...
			[
				'name'      => 'activation_key',
				'title'     => esc_html__( 'Activation Key', 'unconfirmed' ),
				'css_class' => 'activation-key',
			],
			[
				'name'      => 'resent_count',
				'title'     => esc_html__( '# of Times Resent', 'unconfirmed' ),
		...
		if ( ! $this->is_multisite ) {
			$non_ms_cols = [
				[
					'name'  => 'display_name',
					'title' => esc_html__( 'Display Name', 'unconfirmed' ),
				],
			];

			// Can't get array_splice to work right for this multi-d array, so I'm hacking around it.
			$col0      = [ $cols[0] ];
		...
		<form action="<?php echo esc_url( $this->base_url ); ?>" method="post">
		...
		<p class="search-box">
			...
			<?php
			// phpcs:disable WordPress.CSRF.NonceVerification
			$search = ! empty( $_REQUEST['s'] ) ? trim( sanitize_text_field( wp_unslash( $_REQUEST['s'] ) ) ) : '';
			// phpcs:enable
			?>
			<input type="search" id="unconfirmed-search-input" name="s" value="<?php esc_attr( $search ); ?>">
				...
				<tr>
					...
					<td class="login">
						...
						<div class="row-actions">
							...
							?>
							<span class="edit">
								<a class="confirm" href="<?php echo esc_url( $resend_link ); ?>"><?php esc_html_e( 'Resend Activation Email', 'unconfirmed' ) ?></a>
							</span>
							&nbsp;&nbsp;
							<?php
							...
							?>
							<span class="edit">
								<a class="confirm" href="<?php echo esc_url( $activate_link ); ?>"><?php esc_html_e( 'Activate', 'unconfirmed' ); ?></a>
							</span>
							...
							<span class="delete">
								<a title="<?php esc_html_e( 'Deleting a registration means that it will be removed from the database, and the user will be unable to activate his account. Proceed with caution!', 'unconfirmed' ); ?>" class="confirm" href="<?php echo esc_url( $delete_link ); ?>"><?php esc_html_e( 'Delete', 'unconfirmed' ) ?></a>
							</span>
					...
					<td class="activation_key">
						<?php echo absint( $user->resent_count ); ?>
					</td>
